...
const program = new commander.Command(packageJson.name)
  ...
  .option(
    ...
    'use a non-standard version of brahmos-scripts'
  )
  .option('--use-npm')
  .option('--use-pnp')
  .allowUnknownOption()
  .on('--help', () => {
    ...
    console.log(
      `      - a local path relative to the current working directory: ${chalk.green(
        'file:../my-brahmos-scripts'
      )}`
    );
    console.log(
      `      - a .tgz archive: ${chalk.green(
        'https://mysite.com/my-brahmos-scripts-0.8.2.tgz'
      )}`
    );
    console.log(
      `      - a .tar.gz archive: ${chalk.green(
        'https://mysite.com/my-brahmos-scripts-0.8.2.tar.gz'
      )}`
    );
    console.log(
...
function getInstallPackage(version, originalDirectory) {
  let packageToInstall = 'brahmos-scripts';
...
function getPackageName(installPackage) {
  if (installPackage.match(/^.+\.(tgz|tar\.gz)$/)) {
    return getTemporaryDirectory()
      ...
      .catch(err => {
        // The package name could be with or without semver version, e.g. brahmos-scripts-0.2.0-alpha.1.tgz
    ...
    // git+https://github.com/mycompany/brahmos-scripts.git
    // git+ssh://github.com/mycompany/brahmos-scripts.git#v1.2.3
...
// https://github.com/facebook/create-brahmos-app/pull/368#issuecomment-243446094
