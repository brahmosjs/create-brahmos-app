...
inquirer
  ...
  .then(answer => {
    ...
    if (appPackage.devDependencies) {
      // We used to put brahmos-scripts in devDependencies
    ...
    if (fs.existsSync(paths.appTypeDeclarations)) {
      try {
        ...
        // Remove brahmos-scripts reference since they're getting a copy of the types in their project
        content =
          content
            // Remove brahmos-scripts types
            .replace(
              /^\s*\/\/\/\s*<reference\s+types.+?"brahmos-scripts".*\/>.*(?:\n|$)/gm,
              ''
            )
            .trim() + os.EOL;
    ...
    if (ownPath.indexOf(appPath) === 0) {
      try {
        // remove brahmos-scripts and brahmos-scripts binaries from app node_modules
        Object.keys(ownPackage.bin).forEach(binKey => {
    ...
    if (fs.existsSync(paths.yarnLockFile)) {
      ...
      if (process.platform === 'win32') {
        // https://github.com/facebook/create-brahmos-app/pull/3806#issuecomment-357781035
        // Yarn is diligent about cleaning up after itself, but this causes the brahmos-scripts.cmd file
